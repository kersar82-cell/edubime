...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const parentSearchForm = document.getElementById("parent-search-form");
  ...
  if (parentSearchForm) {
    parentSearchForm.addEventListener("submit", async (e) => {
      e.preventDefault();
  ...
  ownerSubTabs.forEach(tab => {
    ...
    // Owner: Assign Class Form
  const ownerAssignForm = document.getElementById("owner-teacher-assign-form");
  if (ownerAssignForm) {
    ownerAssignForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const teacherId = document.getElementById("owner-assign-teacher-select")?.value;
      const classToAssign = document.getElementById("owner-assign-class-input")?.value;

      if (window.EduDB.assignClassToTeacher) {
        await window.EduDB.assignClassToTeacher(teacherId, classToAssign);
